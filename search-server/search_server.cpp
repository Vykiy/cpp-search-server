...
void SearchServer::AddDocument (int document_id, const string &document, DocumentStatus status,
    ...
    for (const string &word: words) {
        ...
        documents_ids_words_fregs_[document_id][word] += inv_word_count;
    }
...
const map<string, double>& SearchServer::GetWordFrequencies(int document_id) const{
    static const map<string, double> words_for_id;
    if(!documents_ids_words_fregs_.count(document_id)){
        return words_for_id;
    }
    return documents_ids_words_fregs_.at(document_id);
}
...
void SearchServer::RemoveDocument(int document_id){
    ...
    if(documents_ids_words_fregs_.find(document_id) != documents_ids_words_fregs_.end()){
        documents_ids_words_fregs_.erase(document_id);
    }
